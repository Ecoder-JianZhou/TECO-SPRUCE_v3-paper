...
999             continue
    ...
    subroutine costFuncObs_update()
        ...
        if(vars4MCMC%gpp_d%existOrNot)then
            call CalculateCost(vars4MCMC%gpp_d%mdData(:,4), vars4MCMC%gpp_d%obsData(:,4)*1.5,&
                 vars4MCMC%gpp_d%obsData(:,5), J_cost)
            J_new(9) = J_new(9) + J_cost!*1000
        endif

        if(vars4MCMC%er_h%existOrNot)then
            call CalculateCost(vars4MCMC%er_h%mdData(:,4), vars4MCMC%er_h%obsData(:,4)*1.5,&
...
999             continue
    ...
    subroutine costFuncObs_old()
        ...
        if(vars4MCMC%cPlant_tree_y%existOrNot)then
            call CalculateCost(vars4MCMC%cPlant_tree_y%mdData(:,4), vars4MCMC%cPlant_tree_y%obsData(:,4),&
                 vars4MCMC%cPlant_tree_y%obsData(:,5), J_cost)
            J_new(1) = J_new(1) + J_cost*100000
        endif
        ! print*, "mod_tree_biomass: ", vars4MCMC%cPlant_tree_y%mdData(:,4)
        ! print*, "obs_tree_biomass: ", vars4MCMC%cPlant_tree_y%obsData(:,4)

        ! ! ANPP_Tree_y
        if(vars4MCMC%ANPP_Tree_y%existOrNot)then
            call CalculateCost(vars4MCMC%ANPP_Tree_y%mdData(:,4), vars4MCMC%ANPP_Tree_y%obsData(:,4),&
                 vars4MCMC%ANPP_Tree_y%obsData(:,5), J_cost)
            J_new(2) = J_new(2) + J_cost*50000
        endif
        ...
        if(vars4MCMC%stem_mass_shrub_y%existOrNot)then
            call CalculateCost(vars4MCMC%stem_mass_shrub_y%mdData(:,4), vars4MCMC%stem_mass_shrub_y%obsData(:,4),&
                 vars4MCMC%stem_mass_shrub_y%obsData(:,5), J_cost)
            J_new(4) = J_new(4) + J_cost*500000
        endif
        ! print*, "mod_stem: ", vars4MCMC%stem_mass_shrub_y%mdData(:,4)
        ! print*, "obs_stem: ", vars4MCMC%stem_mass_shrub_y%obsData(:,4)

        ! ! ! ANPP_Shrub_y
        if(vars4MCMC%ANPP_Shrub_y%existOrNot)then
            call CalculateCost(vars4MCMC%ANPP_Shrub_y%mdData(:,4), vars4MCMC%ANPP_Shrub_y%obsData(:,4),&
                 vars4MCMC%ANPP_Shrub_y%obsData(:,5), J_cost)
            J_new(5) = J_new(5) + J_cost*30000 ! test *10 to better constrain
        endif
        ! print*, "mod_anpp_shrub: ", vars4MCMC%ANPP_Shrub_y%mdData(:,4)
        ! print*, "obs_anpp_shrub: ", vars4MCMC%ANPP_Shrub_y%obsData(:,4)

        ! BNPP_y  ! tree + shrub
        if(vars4MCMC%BNPP_y%existOrNot)then
            call CalculateCost(vars4MCMC%BNPP_y%mdData(:,4), vars4MCMC%BNPP_y%obsData(:,4)*1.1,&
                 vars4MCMC%BNPP_y%obsData(:,5), J_cost)
        ...
        ! print*, "mod_cplant_sphag: ", vars4MCMC%cPlant_sphag_y%mdData(:,4)
        ! print*, "obs_cplant_sphag: ", vars4MCMC%cPlant_sphag_y%obsData(:,4)

        ! ! ! NPP_sphag_y
        if(vars4MCMC%NPP_sphag_y%existOrNot)then
            call CalculateCost(vars4MCMC%NPP_sphag_y%mdData(:,4), vars4MCMC%NPP_sphag_y%obsData(:,4),&
                 vars4MCMC%NPP_sphag_y%obsData(:,5), J_cost)
            J_new(8) = J_new(8) + J_cost*5000
        endif  
        ...
        if(vars4MCMC%gpp_d%existOrNot)then
            call CalculateCost(vars4MCMC%gpp_d%mdData(:,4), vars4MCMC%gpp_d%obsData(:,4),&
                 vars4MCMC%gpp_d%obsData(:,5), J_cost)
            J_new(9) = J_new(9) + J_cost*10000!5000
        endif
...
! enddo
        ...
        if(vars4MCMC%lai_shrub_d%existOrNot)then
            ...
            J_new(17) = J_new(17) + J_cost*10000
        endif
        ! print*, "mod_shrub_lai: ", vars4MCMC%lai_shrub_d%mdData(:,4)
        ! print*, "obs_shrub_lai: ",vars4MCMC%lai_shrub_d%obsData(:,4)
        ...
        if(vars4MCMC%lai_tree_d%existOrNot)then
            ...
            J_new(18) = J_new(18) + J_cost*500000
        endif
        ! print*, "mod_tree_lai: ", vars4MCMC%lai_tree_d%mdData(:,4)
        ! print*, "obs_tree_lai: ", vars4MCMC%lai_tree_d%obsData(:,4)
        ...
        if(vars4MCMC%photo_tree_h%existOrNot)then
            ...
            J_new(19) = J_new(19) + J_cost*100000
        endif
        ! print*, "mod_tree_photo: ", vars4MCMC%photo_tree_h%mdData(:,4)
        ! print*, "obs_tree_photo: ", vars4MCMC%photo_tree_h%obsData(:,4)
